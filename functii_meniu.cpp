...
void meniu_cereri(){
    ...
    while(true){
        afisare_meniu_cereri();
        cin>>opt;

        switch(opt){
            case 1:
                afisare_lista_cereri_valide();
                break;
            case 2:
                afisare_lista_cereri_invalide();
                break;
            case 0:
                return;
        }
...
void afisare_meniu_cereri(){
    cout<<"MENIU CERERI"<<endl<<endl;
    cout << "1. Afisare lista cereri"<<endl;
    cout << "2. Afisare lista cereri invalide"<<endl;
    cout<<"pentru a te intoarce inapoi, apasa tasta 0"<<endl<<endl;
}
...
void adaugare_angajat(){
    ...
    try{
        ...
        if(tip == 1){
            ...
            a = new tehnician(nume, prenume, cnp, d, oras, nr_spec, spec);
...
void afisare_lista_elec_fara_cereri() {
    ...
    cout << "Electrocasnice fara cereri:\n";

    for (size_t i = 0; i < lista_electrocasnice.size(); i++) {
        electrocasnic* e = lista_electrocasnice[i];
        gasit = false;

        for (size_t j = 0; j < lista_cereri.size(); j++) {
            cerere* c = lista_cereri[j];

            if (e->get_tip() == c->get_tip() &&
                e->get_marca() == c->get_marca() &&
                e->get_model() == c->get_model()) {
                gasit = true;
                break;
            }
        }
...
//functii meniu cereri

void afisare_lista_cereri_valide(){
    if(lista_cereri.size() == 0){
        cout << "Nu exista cereri valide.\n";
        return;
    }

    for(int i = 0; i < (int)lista_cereri.size(); i++){
        lista_cereri[i]->afisare(cout);
        cout << "----------------------\n";
    }
}

void afisare_lista_cereri_invalide(){
    if(lista_cereri_invalide.size() == 0){
        cout << "Nu exista cereri invalide.\n";
        return;
    }

    for(int i = 0; i < (int)lista_cereri_invalide.size(); i++){
        cout << lista_cereri_invalide[i] << "\n";
    }
}